-- Supprimer les ateliers existants (optionnel)
-- DELETE FROM workshop_types WHERE slug LIKE '%ikigai%' OR slug LIKE '%icp%' OR slug LIKE '%rule-of%' OR slug LIKE '%messaging%' OR slug LIKE '%brand%' OR slug LIKE '%positionnement%' OR slug LIKE '%autorite%' OR slug LIKE '%business%' OR slug LIKE '%construis-ta-marque%';

-- Insérer les 9 ateliers
...
ON CONFLICT (slug) DO UPDATE SET
  ...
  places_min = EXCLUDED.places_min,
  places_ideal = EXCLUDED.places_ideal,
